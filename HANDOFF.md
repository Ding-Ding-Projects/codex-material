...
- **Releases carry a dim sum code name again.** The index moved to the run number (past 590) while the list it indexed was the 72-dish bundled photo slice, so every build past #72 published unnamed. Names come from `app/dimsum/roster.json` — all 843 catalog dishes, 419 KB — and a missing photo no longer costs a build its name.
...
| --- | --- | --- |
| **The `gh` token has no `read:project` scope** | Confirmed by two independent routes. `gh project list --owner Ding-Ding-Projects` fails with `missing required scopes [read:project]`, and a direct GraphQL query for `organization.projectsV2` returns `INSUFFICIENT_SCOPES`, naming what the token actually holds: `gist, read:org, repo, workflow`. So no GitHub Project can be read, created or updated from here — not through the CLI, and not around it. **This is a recorded decision rather than an open task**: see *Skipped by decision* below, where an earlier session notes the user was asked and chose to skip it. Kept here because the technical evidence belongs with the other external-state entries. | `gh auth refresh -s read:project,project` — an interactive device-code flow against the account's own credentials, which an agent must not run unattended. |
