...
- **Releases carry a dim sum code name again.** The index moved to the run number (past 590) while the list it indexed was the 72-dish bundled photo slice, so every build past #72 published unnamed. Names come from `app/dimsum/roster.json` — all 768 catalog dishes, 374 KB — and a missing photo no longer costs a build its name.
