...
| --- | --- | --- | --- |
| **Chat composer** | `sendChat` — the ↑ button, <kbd>Enter</kbd> without <kbd>Shift</kbd> (`onChatKey`), or the slash panel's **Run now** (`slashRun`) | `["exec"]` + `profileArgv()` + `[prompt]` | the reply bubble at `messages[replyIndex]` |
...
| Send key | `onChatKey` | <kbd>Enter</kbd> sends, <kbd>Shift</kbd>+<kbd>Enter</kbd> newlines |
...
[../architecture/ipc-bridge.md](../architecture/ipc-bridge.md#security-considerations): under
...
- **Everything composed here reaches a shell on Windows.** `shell: WIN` plus Node's
  ...
  [../architecture/ipc-bridge.md](../architecture/ipc-bridge.md#security-considerations).
