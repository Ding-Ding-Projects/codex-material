...
not load, a plugin install, a WSL exec result — is a toast. See [tabs.md](tabs.md) for the gate's
preview contract.
...
- The entrance animation is a 0.18 s `cxin` keyframe; **it is not currently gated on
  `prefers-reduced-motion`** — see [../experience/accessibility.md](../experience/accessibility.md).
