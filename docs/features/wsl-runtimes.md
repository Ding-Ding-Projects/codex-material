...
[tabs.md](tabs.md)), but neither carries a WSL binding. `CX.sim.wsl` is refreshed from
...
[chats-and-runs.md](chats-and-runs.md) — because `startRun` invokes `cli.stream(cli.codexBin(), …)`
...
6. **Stop vs kill:** stop an instance — the row must remain and report `stopped`. Remove it — the
   row must go.
7. **Shutdown:** spawn several, quit Studio, then run `wsl.exe -d <distro> -- pgrep -fa "sleep
