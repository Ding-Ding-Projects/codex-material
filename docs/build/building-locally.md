...
| `npm test` | `node tools/test-frontend.mjs && node tools/test-backend.mjs && node tools/sync-changelog.mjs --check` | 34 frontend module tests, 33 backend tests, and a check that `app/CHANGELOG.md` still matches the root copy. |
...
3. `npm test` passes: **34 frontend tests**, **33 backend tests**, and
