...
and no socket — see [../api/README.md](../api/README.md).
...
`app/codex-core.js` — see [frontend-runtime.md](frontend-runtime.md#browser-preview-mode).
...
verbatim (see [../features/notifications.md](../features/notifications.md)). Do not pattern-match
...
command`) and runs `codex mcp add <name> -- <command> [args…]`. Any other transport requires `url`
...
See [../features/wsl-runtimes.md](../features/wsl-runtimes.md). `session` is the tab id and is the
...
never pushed. See [../features/local-version-control.md](../features/local-version-control.md).
...
- **The renderer has no filesystem and no network of its own.** `contextIsolation` plus
  ...
  [frontend-runtime.md](frontend-runtime.md#why-unsafe-eval-is-in-the-csp).
