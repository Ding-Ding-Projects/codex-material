...
``<script data-dc-script> must define `class Component extends DCLogic` `` and renders nothing.
...
- The renderer has no Node and no network. Everything it can do to the machine is on the command
  list in [ipc-bridge.md](ipc-bridge.md); argv it composes is spawned through a shell on Windows,
