...
(function (g) {
  ...
  var ARTICLES = [
    ...
    a("appearance", "Per-element appearance customization", "Feature",
      "41 named appearance targets, an anchored non-modal editor, and a colour translator across HEX, RGB, HSL, HSV, HWB, LAB, LCH, OKLab, OKLCH and CMYK with a WCAG contrast ratio.",
      {
        en: [
    ...
    a("site", "This site, and why it carries the app's features", "Feature",
      ...
      [
        { h: "Behaviour", blocks: [
          ...
          p("**A per-element appearance editor.** Right-click any named surface for *Edit appearance…*, or <kbd>Shift</kbd>+right-click to skip the menu: typeface, slant, capitalization, underline, size, weight, letter spacing, text colour and highlight, applied live and kept per element. The sheet it lives in is itself a named target, so the appearance system can restyle its own chrome — a theming feature that cannot theme its own dialog is incomplete. Only properties this system set on the previous pass are cleared, because writing an empty value for every property it knows about erases the stylesheet's own: the app did that once and its UI audit went from 23 findings to 251 while every other test stayed green."),
          p("**A search on the settings surface**, wired to its own regex builder — the instructions ask for one on every adjustment surface, and \"it is a short page, just scroll\" is not an answer for a reader who knows a setting's name but not which card it is on. It matches each row's label, its description **and its current value**, so typing `cosy` finds the density row by what it is set to rather than by what it is called. A card whose every row is hidden is hidden too."),
          p("**Append-only version history** over every preference this page owns. A settings page is a record store however small it looks: clearing an accent you spent five minutes choosing is exactly what an undo exists for. Recorded at the single funnel every write passes through, so a future call site cannot forget; an unchanged write records nothing; and each row says what changed rather than that something did. **Restoring writes a new revision rather than rewinding**, so an undo can itself be undone. Filter by a date range, by text, or by actions derived from the log with a count beside each — a hard-coded list offers choices the log does not contain and misses the ones it does.")
        ]},
        { h: "Configuration", blocks: [
