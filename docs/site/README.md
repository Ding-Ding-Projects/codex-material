...
`tablist` / `tab` / `tabpanel` structure and roving `tabindex`. <kbd>←</kbd> <kbd>→</kbd>
<kbd>Home</kbd> <kbd>End</kbd> move between them.
...
- Pin a tab with <kbd>P</kbd>, reload, and it is still pinned and still first.
