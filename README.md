...
[![CI](https://github.com/Ding-Ding-Projects/codex-material/actions/workflows/ci.yml/badge.svg)](https://github.com/Ding-Ding-Projects/codex-material/actions/workflows/ci.yml)
...
Studio resolves the `codex` binary once per process, in this order ([`electron/lib/cli.js`](electron/lib/cli.js), `resolveCodex()`):
...
**The honest size cost:** the bundled tree is **~409 MB unpacked** (429,285,783 bytes for `codex-cli 0.146.0`, per [`vendor/codex-bin-version.json`](vendor/codex-bin-version.json)). It is not a lone executable — the CLI resolves `codex-path/rg.exe`, `codex-resources/…` and the code-mode host relative to itself, so the whole platform tree ships or none of it does. That single directory is the largest thing in the installer by an order of magnitude, and it is why a Material 3 GUI needs half a gigabyte. It is staged by [`tools/fetch-codex.mjs`](tools/fetch-codex.mjs) from OpenAI's own published npm artifact (`@openai/codex@<version>-win32-x64`), not a mirror.
...
<details open>
<summary><b>Driving the CLI</b></summary>
...
<details open>
<summary><b>Studio's own surfaces</b></summary>
...
- **Command palette** — <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>F</kbd>, itself searchable with the regex builder.
...
[`.github/workflows/ci.yml`](.github/workflows/ci.yml) runs on every `push` and on `workflow_dispatch`, on `windows-latest`.
...
<details open>
<summary><b>Directory table</b></summary>
...
Feature documentation lives under [`docs/`](docs/), one file per feature beneath a categorised folder, each with a `README.md` index — Architecture, Build, Features, Experience and API. The same tree is published as the project site. See [Known limitations](#known-limitations) for which pages are currently behind the code.
...
<details>
<summary><b>Read the mirrored instructions</b></summary>
