...
Codex Studio is a Windows-only Electron desktop app that drives the OpenAI Codex CLI. Read [`README.md`](README.md) for what it is and how to build it; this file is the part an agent or contributor needs before touching the tree.

## Contents

| Section | What is there |
| --- | --- |
| [Orientation](#orientation) | The architecture in five sentences |
| [Never touch](#never-touch) | Generated files, frozen exports, and out-of-scope surfaces |
| [House rules](#house-rules) | Conventions specific to this repository |
| [How to add a backend command](#how-to-add-a-backend-command) | Preload allow-list → `commands.js` → the test that keeps them in step |
| [How to add a UI panel](#how-to-add-a-ui-panel) | Template block → `*Vals()` → `NAV` entry |
| [How to add an i18n key](#how-to-add-an-i18n-key) | Five levels per language, placeholders that survive all five |
| [How to add a search bar](#how-to-add-a-search-bar) | The `<target>Query` / `<target>Regex` / `data-anchor` convention |
| [Before you call it done](#before-you-call-it-done) | The checks that must pass |
...
The **renderer** is `app/index.html`: one `<x-dc>` template block followed by an inline `<script type="text/x-dc" data-dc-script>` holding `class Component extends DCLogic`, compiled at load by the generated dc runtime in `app/support.js` (a React-backed template engine). The **frontend runtime** is `app/codex-core.js`, which assigns `window.CX` — the backend bridge, the `localStorage` store, a TOML writer, the bounded regex engine, colour translation, i18n, the narrator, the local version history, notifications, tabs, settings, the dim sum draw, and a full in-page **simulation** of the backend so the same build opens as a design preview in a plain browser. The **backend** is `electron/main.js` (window and lifecycle), `electron/preload.js` (an allow-list of 50 named IPC commands and nothing else) and `electron/commands.js` (their handlers), delegating to six small modules in `electron/lib/` — `cli`, `config`, `catalog`, `wsl`, `history`, `editors`. Every one of those handlers either runs the real `codex` binary and hands its output back verbatim, or reads and writes a real file under `$CODEX_HOME`; none of them models the agent, the sandbox, the approval semantics or the config schema. The `tools/` directory holds everything that is not the app: two test runners, the headless screenshot harness, the CLI staging script, the changelog mirror, the site-asset mirror, the dim sum catalog sync, the release code-name resolver and the icon generator.

```
app/          renderer — plain browser JS, no build step, no npm deps
electron/     main process, preload allow-list, IPC handlers, lib modules
tools/        tests, capture harness, CLI staging, generators
docs/         categorised documentation, also published as the project site
assets/       icons and the captured screenshots
design/       frozen design-tool export the app was grown from
vendor/       openai/codex submodule (reference) and the staged CLI (git-ignored)
```
...
| Path | Why |
| --- | --- |
| `app/support.js` | Generated from the dc-runtime sources. Hand edits are lost on the next regeneration and there is no way to tell they happened. |
| `app/CHANGELOG.md` | A mirror of the root `CHANGELOG.md`, written by `tools/sync-changelog.mjs`. Edit the root file; `npm test` fails when the two drift. |
| `app/cx-dimsum.js`, `app/dimsum/` | Generated by `tools/sync-dimsum.ps1` from the shared dim sum catalog. |
| `design/` | The original design-tool export the app was grown from. Reference only, never loaded at runtime, never edited. |
| `vendor/codex` | A submodule pointing at `openai/codex`, present so the CLI's own source can be read while transcribing flags and config keys. Nothing here builds it, and CI checks out with `submodules: false`. |
| `vendor/codex-bin/` | Staged build output. Git-ignored; produced by `npm run prepare:cli`. |

---

## House rules

**`app/` is plain browser JavaScript.** No bundler, no transpiler, no import statements, no `node_modules` at runtime. Each feature module is an IIFE that assigns to a `window.CX_*` global (`CX_I18N`, `CX_TABS`, `CX_NOTIFY`, `CX_CHANGELOG`, `CX_DIMSUM`) and is loaded by a `<script src>` in the `<helmet>` block of `app/index.html`. A new module is a new file plus a new `<script src>` line — nothing else.

**Nothing is fetched at runtime.** The CSP is `default-src 'self'` with `connect-src 'self'`. React, both Roboto families, the dim sum photographs and the changelog are all bundled. If a change needs an asset, the asset goes into `app/`.

**`'unsafe-eval'` in the CSP is load-bearing.** The dc runtime compiles this page's own template and logic with `new Function`. It compiles code that is already in the bundle and never anything fetched, and the comment above the CSP meta tag says so. Do not "fix" it by removing the directive — the window renders blank.

**The frontend must work in both modes.** Under Electron `window.CODEX_BRIDGE` exists; opened as a file in a browser it does not, and `CX.bridge.invoke` falls through to the simulation in `codex-core.js`. Always call `CX.bridge.invoke(...)` and `CX.bridge.listen(...)`; never reach for `window.CODEX_BRIDGE` directly, and never assume a call reached a real process. `CX.bridge.mode` is `"electron"` or `"browser"`, and the title bar shows which.

**The simulation is a development fallback, not a data source.** `CX.live.hydrate()` replaces the simulated state object *in place* with what the real CLI reported, so a panel that reads `CX.sim.mcp` keeps working and starts showing real data. If you add a panel that reads `CX.sim.<something>`, make sure `codex_state` in `electron/commands.js` and `live.hydrate` in `codex-core.js` actually populate it — otherwise the panel ships showing invented data, which is worse than showing nothing.

**The backend is CommonJS with `"use strict"`,** using only Node built-ins plus `smol-toml`. Keep it that way; the renderer has no dependencies at all and the main process has one.

**Errors carry the CLI's own words.** `command()` in `electron/commands.js` rethrows with the real message, and the renderer surfaces it verbatim through `CX.notifyBackendFailure(what, err)`. Never replace a real failure with a generic one, and never let a section fail silently — `codex_state` collects per-section errors and the UI warns by name, because an empty list reads as "you have none", which is a different fact.

**Windows process spawning has two rules.** `codex` resolves to a `.cmd` shim that `spawn` will not execute without a shell, so everything goes through `shell: true` on win32 — which means arguments must stay an argv **array** and must never be interpolated into a command string. And both stdout and stderr are drained concurrently; draining one to completion before touching the other deadlocks the moment a chatty process fills the pipe nobody is reading.

**Scope.** This repository is the Windows desktop app. There is no macOS or Linux target, and adding one is a product decision, not a refactor.
...
**1. `electron/lib/<module>.js`** — put the actual work in the right module (`cli`, `config`, `catalog`, `wsl`, `history`, `editors`) or add a new one. Throw a real `Error` with the CLI's message on failure.

**2. `electron/commands.js`** — register the handler. The name must match the regex the test uses: lowercase letters and underscores, registered with a literal string at the start of a line.

```js
command("codex_thing_list", async (a) => catalog.thingList(a.cwd || null));
```

**3. `electron/preload.js`** — add the name to the `COMMANDS` array, as its own line, indented two spaces, quoted, with a trailing comma:

```js
  "codex_thing_list",
```

That exact shape matters: `tools/test-backend.mjs` extracts the allow-list with `/^\s{2}"([a-z_]+)",$/gm` and the registrations with `/^command\("([a-z_]+)"/gm`, then asserts every exposed name is registered. A name in one list and not the other is a command the UI can call and the backend will refuse — a mystery error at runtime — so the test exists to make that impossible to merge.

Then call it from the frontend as `CX.bridge.invoke("codex_thing_list", { cwd })`. If it should also work in browser preview, add a `case` to the `sim()` switch in `app/codex-core.js`.

For a **streaming** command, send on a `codex://`-prefixed channel (`preload.js` refuses to listen on anything else) and route the lines through the existing `this._runs` map and `startRun()` in `app/index.html`, so every surface gets live output rather than a wait-then-dump.

Add a test in `tools/test-backend.mjs` for the module function itself — the allow-list test only proves the wiring.

---

## How to add a UI panel

Three edits, all in `app/index.html`.

**1. The `NAV` entry** (top of the `data-dc-script` block) adds the rail item and, for free, a command-palette row:

```js
const NAV = [
  …
  { id: "thing", icon: "◆", label: "Thing", hint: "One line explaining what this panel is for" }
];
```

**2. The template block** goes inside the main content area of the `<x-dc>` template, gated on a boolean the logic supplies:

```html
<sc-if value="{{ isThing }}" hint-placeholder-val="{{ false }}">
  <div data-appear="Thing panel" onContextMenu="{{ thingMenu }}"> … </div>
</sc-if>
```

Use `<sc-for list="{{ rows }}" as="r">` for repetition, `{{ … }}` for every binding, and Material 3 CSS custom properties (`var(--m3-surface-container)`, `var(--m3-on-surface-variant)`, …) rather than literal colours — the light theme is a token swap and a hard-coded hex breaks it.

**3. The bindings.** Small panels can add their keys directly to the object `renderVals()` returns. Anything substantial gets its own method and is merged in at the bottom of `renderVals()`:

```js
  thingVals() {
    const st = this.state;
    return {
      isThing: st.nav === "thing",
      thingRows: […],
      openThing: () => this.setState({ … })
    };
  }
```

```js
    Object.assign(v, this.tabVals(), this.notifyVals(), this.changelogVals(), this.studioVals(), this.dimSumVals(), this.thingVals());
```

Two conventions that are not optional:

- **`data-appear="Some name"`** on every element a user might want to restyle. That attribute is what makes the element a target: `applyAppearance()` walks `[data-appear]` and applies the stored override, and `appearItem(e)` finds the nearest one so *Edit appearance…* can name it. An element with no `data-appear` cannot be customized, which the appearance rules do not allow.
- **A context menu** built with `this.menuAt(e, title, items)`, ending with `this.appearItem(e)` and including `this.copyItem(text)` where copying makes sense.

Anything the panel *informs* the user about goes through `this.toast(text, kind)` or `CX.notify` — a corner notification, not a dialog. A modal is only correct for a decision that must be made before continuing; the bulk-close preview is the one place in this app that qualifies.

---

## How to add an i18n key

Add it to `STRINGS` in `app/cx-i18n.js`. **Every key needs exactly five entries per language, in both languages** — no exceptions, no shorthand, no single string:

```js
    "thing.saved": {
      en: ["Saved {name}.", "Saved {name}.", "Saved {name}.",
        "Saved {name} — it is on disk now.", "Saved {name}. It is on disk, go on with your day."],
      yue: ["已儲存 {name}。", "已儲存 {name}。", "存咗 {name}。",
        "存咗 {name} — 已經寫入磁碟。", "存咗 {name} 落磁碟，可以放心。"]
    },
```

Index 0 is funny level 1 (fully professional) and index 4 is level 5 (maximum playfulness). `tools/test-frontend.mjs` enforces all of this:

- every key resolves to exactly 5 non-blank levels in both `en` and `yue`;
- at least one key per language differs between level 1 and level 5, so the slider is not inert;
- **no key drops a `{placeholder}` as the level rises** — whatever appears in level 1 must appear in levels 2 to 5;
- **error, warning, confirmation, security and destructive keys are checked especially.** Any key matching `err`, `error`, `warn`, `warning`, `danger`, `destructive`, `confirm`, `security`, `fail` or `failed` as a prefix or a dotted segment must keep every placeholder at every level. This is the voice-not-facts rule made mechanical: the joke may change, `{detail}`, `{path}`, `{count}` and `{code}` may not.
- bilingual mode joins both sides at every level, so a key whose two languages read identically is fine but a key with an empty side is not.

Render with `CX.i18n.t("thing.saved", { name })`. Interpolation only happens when you pass a vars object — `CX.i18n.t("err.foo")` without one returns the literal `{detail}`, which is why `notifyBackendFailure` always passes it. An unknown key renders as the key itself, deliberately: a missing string must be visible, not blank.

A backend failure category gets an `err.<what>` key so `CX.notifyBackendFailure("<what>", err)` finds it.

---

## How to add a search bar

The regex builder belongs to the field, not to a menu. The convention in `app/index.html` is:

1. Two state keys named after a `target` string: `st.<target>Query` (plain text, the default) and `st.<target>Regex` (`{ pattern, flags }`, or `null`).
2. `data-anchor="<target>"` on the input element, so `openRegexFor(target)` can anchor the popover to that exact field.
3. Filter with `this.matcher(st.<target>Query, st.<target>Regex)`, which returns a predicate. It falls back to a case-insensitive substring test when no pattern is set and to *match everything* when a pattern is invalid, so a half-typed regex never blanks the list.
4. Add a case to `sampleFor(target)` so the builder opens with realistic sample text from that surface.
5. Put `this.filterItems(target, text)` into the row context menus, which gives the surface filter, exclude, starts-with, exact-match, open-in-builder and clear for free.

Every settings, properties and adjustment surface needs one of these too — not only list panels.
...
```powershell
npm test          # frontend modules, backend modules, changelog mirror
npm run capture   # if a visible surface changed
```

- `npm test` runs the two suites and the changelog mirror check. CI's test job runs those **plus** `node tools/sync-site-assets.mjs --check` and a `node --check` sweep over every `.js`/`.mjs`/`.cjs` in `app/`, `electron/` and `tools/` — run `node --check <file>` on anything you touched that the unit tests do not load.
- A release is published only when the test job passes, so a red test ships nothing.
- If you changed a visible surface, re-capture it. The harness runs the real app headlessly through the real IPC handlers, so a capture is evidence and a mockup is not.
- Update `CHANGELOG.md` (the root one) and the affected page under `docs/`. Record what actually changed and what remains unverified; never write an entry for work that is not in the tree.
- Read the mirrored instructions below before finishing — the Git, GitHub, issue-triage and release rules apply to this repository.
