...
- `npm test` is exactly what CI's test job runs, minus the `node --check` sweep over every `.js`/`.mjs`/`.cjs` in `app/`, `electron/` and `tools/` — run `node --check <file>` on anything you touched that the unit tests do not load.
